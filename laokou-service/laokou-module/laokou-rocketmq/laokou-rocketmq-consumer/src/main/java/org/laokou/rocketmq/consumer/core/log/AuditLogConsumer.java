...
@Slf4j
public class AuditLogConsumer implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt message) {
        ...
        try {
            final AuditLogDTO auditLogDTO = JacksonUtil.toBean(messageBody, AuditLogDTO.class);
            HttpResultUtil<Boolean> result = logApiFeignClient.audit(auditLogDTO);
            if (result.success()) {

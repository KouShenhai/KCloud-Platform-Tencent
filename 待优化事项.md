...
- [ ] 审批 > 审批日志表
- [ ] 同步 > mongodb日志
- [ ] 请求链 > kafka > logstash > es
- [ ] 同步数据时带上时间戳（key = 业务key + 时间戳），防止redis出现死锁（分布式锁相关内容）
- [ ] [spEL学习](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#expressions)
- [ ] springsecurity原理学习 > 构建springsecurity + jwt认证
- [ ] java架构之道，往架构方向不断前进，绘图，原理，源码，设计